...
#         print('covar:', cmaes.C)
        ...
        num_init_samples = 10
#         points = latin_hypercube(num_init_samples, func.dims)
#         points = from_unit_cube(points, func.lb, func.ub)
#         train_x, train_y = [], []
#         for i in range(num_init_samples):
#             train_x.append(points[i])
#             train_y.append(func(points[i]))
    
        train_x = [np.random.uniform(func.lb, func.ub) for _ in range(num_init_samples)]
        train_y = [func(train_x[i]) for i in range(num_init_samples)]
            
        cmaes.tell(train_x, train_y)
        # fitness_list.extend(fitness)
        # best_eval.append(max(fitness_list))
        print('covar:', cmaes.C)
